...
public:
    ...
    void create_tree(node* root0, vector<int> &v){
        ...
        root0->left=newNode(v[1]);
        root0->right=newNode(v[2]);
        
        root0->left->left = newNode(v[3]);
        root0->left->right = newNode(v[4]);
        
        root0->right->left = newNode(v[5]);
        root0->right->right = newNode(v[6]);
    }
